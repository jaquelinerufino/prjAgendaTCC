    ...
    public partial class frmNovoEvento : Form
    {
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            if (mtxtHoraInicial.MaskCompleted == true && mtxtHoraFinal.MaskCompleted == true && cmbTipoEvento.Text != "" && txtNome.Text != "")
            {
                ...
                {
                    MessageBox.Show("Atenção: digite a hora corretamente! (00:00 - 23:59)");
                    return;
                }

                if (horarioI == horarioF)
                {
                    MessageBox.Show("Atenção: os horarios não podem ser iguais!");
                    return;
                }

                if (int.Parse(horarioI) > int.Parse(horarioF))
                {
                    MessageBox.Show("Atenção: A hora de inicio não pode ser maior que á de termino!");
                    return;
                }
                ...
                {
                    MessageBox.Show("Atenção: Digite uma data válida!");
                    return;
                }

                if (DataTermVal == false && mtxtFinal.ReadOnly == false)
                {
                    MessageBox.Show("Atenção: Digite uma data de término válida!");
                    return;
                }
                ...
                if (DataI.Date == System.DateTime.Today.Date)
                {
                    ...
                    {
                        MessageBox.Show("Atenção: Horário de inicio já passou!");
                        return;
                    }

                    if (TimeSpan.Parse(horarioF) < System.DateTime.Now.TimeOfDay && mtxtHoraFinal.ReadOnly == false)
                    {
                        MessageBox.Show("Atenção: Horário de término já passou!");
                        return;
                    }
                }
                ...
                if (btnSalvar.Text == "Salvar")
                {
                    ...
                    else
                    {
                        ...
                        Conexao.AdicionarParametroTexto(horarioI);
                        Conexao.AdicionarParametroTexto(DataF.ToString("yyyy-MM-dd"));
                        Conexao.AdicionarParametroTexto(horarioF);
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show("Para salvar a edição é preciso que o campo de 'valor arrecadado' seja preenchido");
                        return;
                    }
                    else
                    {
                        for (int i = 0; i < txtValor.Text.Length; i++)
                        {
                            if (txtValor.Text.Substring(i, 1) == "," || txtValor.Text == "")
                            {
                                ...
                                try
                                {
                                    ...
                                    MessageBox.Show("Atenção: O formato do valor arrecadado está errado.");
                                    return;
                                }
                                catch
                                {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Digite um tipo de evento para cadastra-lo!");
                return;
            }

            if (cmbTipoEvento.SelectedIndex != -1)
            {
                MessageBox.Show("Este evento já está cadastrado!");
                return;
            }

            for (int i = 0; i < cmbTipoEvento.Items.Count; i++)
            {
                if (cmbTipoEvento.Items[i].ToString() == cmbTipoEvento.Text)
                {
                    MessageBox.Show("Atenção: Já á um tipo de evento com este nome!");
                    return;
                }
            }
        ...
        private void btnExc_Click(object sender, EventArgs e)
        {
            if (cmbTipoEvento.Text == "")
            {
                MessageBox.Show("Escolha um tipo de evento para exclui-lo!");
                return;
            }

            if (cmbTipoEvento.SelectedIndex == -1)
            {
                MessageBox.Show("Este evento não está cadastrado!");
                return;
            }
            ...
            for(i=0; i<TE.QtdCodigos1(); i++)
            {
                ...
                {
                    MessageBox.Show("Atenção: há um ou mais eventos cadastrados" + "\n" + "com o tipo de evento: '" + cmbTipoEvento.Text + "'." + "\n" + "É necessário a exclusão deles antes de executar" + "\n" + "está operação!", this.Text);
                    return;
                }
            }
        ...
        private void mtxtInicio_TextChanged(object sender, EventArgs e)
        {
            ...
            if (mtxtInicio.ReadOnly == false)
            {
                if (mtxtInicio.MaskCompleted == true)
                {
                    ...
                    {
                        MessageBox.Show("Atenção: A data " + mtxtInicio.Text + " já passou!", this.Text);
                        mtxtInicio.Focus();
                        DataValida = false;
                        //btnSalvar.Enabled = false;
                        return;
        ...
        private void mtxtFinal_TextChanged(object sender, EventArgs e)
        {
            ...
            if (mtxtFinal.ReadOnly == false)
            {
                if (mtxtFinal.MaskCompleted == true)
                {
                    ...
                    {
                        MessageBox.Show("Atenção: A data " + mtxtFinal.Text + " já passou!", this.Text);
                        mtxtFinal.Focus();
                        DataTermVal = false;
                        return;
                    }
                    ...
                    {
                        MessageBox.Show("Atenção: A data de vecimento deve ser depois que a data da vistoria.");
                        mtxtFinal.Focus();
                        DataTermVal = false;
                        return;
                    }
        ...
        private void frmNovoEvento_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            //if (caminho == 1)
            //{
            //    #region Recarrega a lista de eventos

            //    evento = (frmAgenda)this.Owner;

            //    if (btnSalvar.Text != "Salvar")
            //    {
            //        lblValorArrecadado.Visible = false;
            //        lblRS.Visible = false;
            //        txtValor.Visible = false;
            //        txtValor.Text = "";


            //        //evento.lstEventos.Items.Clear();
            //        evento.lstData.Clear();
            //        evento.lstHora.Clear();
            //        evento.lstValor.Clear();
            //        evento.lstDataFn.Clear();
            //        evento.dgvListas.Rows.Clear();
            ...
            //        while (DR.Read())
            //        {
            //            evento.lstData.Add(DR[0].ToString());
            //            evento.lstHora.Add(DR[1].ToString());
            //            evento.lstDataFn.Add(DR[2].ToString());

            //            if (DR[7].ToString() != "")
            //            {
            //                evento.lstValor.Add(DR[7].ToString());
            //            }
            //            else
            //            {
            //                evento.lstValor.Add("");
            ...
            //            evento.dgvListas.Rows.Add(DR.GetDate(0).ToString("dd/MM/yyyy"), DR.GetDate(2).ToString("dd/MM/yyyy"), DR.GetString(4));
            //        }
            ...
            //        string Horafn = (evento.monthCalendar1.TodayDate.Hour).ToString() + ":" + (evento.monthCalendar1.TodayDate.Minute).ToString();

            //        for (int i = 0; i < evento.dgvListas.Rows.Count; i++)
            //        {
            //            if (DateTime.Parse(evento.lstDataFn[i]) < evento.monthCalendar1.TodayDate && evento.lstValor[i] == "" || DateTime.Parse(evento.lstDataFn[i]) == evento.monthCalendar1.TodayDate && evento.lstValor[i] == "" && DateTime.Parse("01/01/0001" + evento.lstHoraFn[i]) < DateTime.Parse("01/01/0001" + Horafn))
            //            {
            //                evento.dgvListas.Rows[i].DefaultCellStyle.BackColor = Color.Yellow;
            //            }
            //        }
            //    }
            //    #endregion
