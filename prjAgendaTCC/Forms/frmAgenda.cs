    ...
    public partial class frmAgenda : Form
    {
        ...
        private void frmAgenda_Load(object sender, EventArgs e)
        {
            ...
            while (DR.Read())
            {
                ...
                dgvListas.Rows.Add(DR.GetDateTime(0).ToString("dd/MM/yyyy"), DR.GetDateTime(2).ToString("dd/MM/yyyy"), DR.GetString(4));
            }
            ...
            //{
            //    if (DateTime.Parse(lstDataFn[i]) < monthCalendar1.TodayDate && lstValor[i] == "" || DateTime.Parse(lstDataFn[i]) == monthCalendar1.TodayDate && lstValor[i] == "" && DateTime.Parse("01/01/0001" + lstHoraFn[i]) < DateTime.Parse("01/01/0001" + Horafn))
            //    {
        ...
        private void dgvListas_SelectionChanged(object sender, EventArgs e)
        {
            if (rdbEvento.Checked)
            {
                ...
                if (dgvListas.CurrentRow.Index >= 0 && lstData.Count > 0)
                {
                    ...
                    Conexao.AdicionarParametroTexto("@DataE", DateTime.Parse(lstData[dgvListas.CurrentRow.Index]).Date.ToString("yyyy-MM-dd"));
                    Conexao.AdicionarParametroTexto("@Hora", lstHora[dgvListas.CurrentRow.Index]);
                    ...
                    if (dgvAnimais.Rows.Count == 0)
                    {
                        ...
                        btnNovaRelacao.Text = "A&dicionar Relação";
                    ...
                    if (DateTime.Parse(lstData[dgvListas.CurrentRow.Index]) < monthCalendar1.TodayDate)
                    {
                        btnNovaRelacao.Enabled = false;
                    }
                    else
            ...
            else
            {
                ...
                if (rdbVistoria.Checked)
                {
                    ...
                    if (dgvListas.Rows.Count > 0 && lstCdPessoa.Count > 0)
                    {
                        ...
                        Conexao.AdicionarParametroInteiro("@Pessoa", lstCdPessoa[dgvListas.CurrentRow.Index]);
                        LstAnimais = Conexao.ChamarStoredProcedureCR();
        ...
        private void rdbVistoria_CheckedChanged(object sender, EventArgs e)
        {
            if (rdbVistoria.Checked == true)
            {
                ...
                while (DR.Read())
                {
                    ...
                    if (DR.GetDateTime(0) == monthCalendar1.TodayDate && DR.GetTimeSpan(2) < TimeSpan.Parse(hora) || DR.GetDateTime(0) < monthCalendar1.TodayDate)
                    {
                        if (DR[4].ToString() == "" && DR[5].ToString() == "")
                        {
                            if (DR[7].ToString() != "")
                            {
                                if (DR[7].ToString() != "1")
                                {
                                    dgvListas.Rows.Add(DR.GetDateTime(0).ToString("dd/MM/yyyy"), DR[1].ToString(), "Dono de um abrigo inativo");
                                }
                                else
                                {
                                    dgvListas.Rows.Add(DR.GetDateTime(0).ToString("dd/MM/yyyy"), DR[1].ToString(), "Dono de um abrigo Ativo");
                                }
                            }
                            else
                            {
                                if (DR[8].ToString() != "")
                                {
                                    if (DR[7].ToString() != "1")
                                    {
                                        dgvListas.Rows.Add(DR.GetDateTime(0).ToString("dd/MM/yyyy"), DR[1].ToString(), "Adotante desbloqueado");
                                    }
                                    else
                                    {
                                        dgvListas.Rows.Add(DR.GetDateTime(0).ToString("dd/MM/yyyy"), DR[1].ToString(), "Adotante bloqueado");
                                    }
                                }
                                else
                                {
                                    dgvListas.Rows.Add(DR.GetDateTime(0).ToString("dd/MM/yyyy"), DR[1].ToString(), "");
                                }
                            }
        ...
        private void btnExcluir_Click(object sender, EventArgs e)
        {
            ...
            if (rdbEvento.Checked)
            {
                if (MessageBox.Show("Você deseja realmente excluir este evento?", this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    ...
                    while (DR.Read())
                    {
                        //lstEventos.Items.Add(DR.GetDateTime(0).ToString("dd/MM/yyyy") + " Até " + DR.GetDateTime(2).ToString("dd/MM/yyyy") + " - " + DR.GetString(4));
                        lstData.Add(DR[0].ToString());
                        lstHora.Add(DR.GetTimeSpan(1).ToString());
                    ...
                    for (int i = 0; i < dgvListas.Rows.Count; i++)
                    {
                        if (DateTime.Parse(lstData[i]) < monthCalendar1.TodayDate && lstValor[i] == "")
                        {
                            this.dgvListas.Rows[i].DefaultCellStyle.BackColor = Color.Yellow;
                        }
                    }
            ...
            else
            {
                if (MessageBox.Show("Você deseja realmente excluir esta vistoria?", this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    ...
                    MessageBox.Show("Vistoria excluido com sucesso!");
        ...
        private void btnNovaRelacao_Click(object sender, EventArgs e)
        {
            ...
            while (DR.Read())
            {
                ...
                {
                    RelAn.lblEvento.Text = DR.GetDateTime(0).ToString("dd/MM/yyyy") + " até " + DR.GetDateTime(2).ToString("dd/MM/yyyy") + " - " + DR[4].ToString();
                }
            }
        ...
        {
            controlarTamanhoGrid(dgvListas, 41, 40, 370, 238);
        }

        private void dgvAnimais_Paint(object sender, PaintEventArgs e)
        {
            controlarTamanhoGrid(dgvListas, 41, 40, 287, 238);
        }
        #endregion
        ...
        public void monthCalendar1_DateChanged(object sender, DateRangeEventArgs e)
        {
            ...
            if (rdbEvento.Checked == true)
            {
                if (monthCalendar1.TodayDate > DateTime.Parse(Inicio))
                {
        ...
        private void btnNovoEvento_Click(object sender, EventArgs e)
        {
            ...
            //    for (int i = 0; i < dgvListas.Rows.Count; i++)
            //    {
            //        if (DateTime.Parse(lstDataFn[i]) < monthCalendar1.TodayDate && lstValor[i] == "" || DateTime.Parse(lstDataFn[i]) == monthCalendar1.TodayDate && lstValor[i] == "" && TimeSpan.Parse(lstHoraFn[i]) < TimeSpan.Parse(Horafn))
            //        {
            //            this.dgvListas.Rows[i].DefaultCellStyle.BackColor = Color.Yellow;
            //        }
            //    }
