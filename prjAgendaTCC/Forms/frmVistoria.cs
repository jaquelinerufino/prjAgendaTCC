...
namespace prjAgendaTCC
{
    public partial class frmVistoria : Form
    {
            ...
            else
            {
                ...
                while (DR.Read())
                {
                    ...
                    if (DateTime.Parse(vistoria.lstDataVis[vistoria.dgvListas.CurrentRow.Index]).Date.ToString("yyyy-MM-dd") == DR.GetString(0) && vistoria.lstHoraVis[vistoria.dgvListas.CurrentRow.Index] == DR[2].ToString() && vistoria.lstCdPessoa[vistoria.dgvListas.CurrentRow.Index] == DR.GetInt32(3))
                    {
                        ...
                        if (DR[6].ToString() != "")
                        {
                            mtxtDataVencimento.Text = DR.GetDateTimeTime(6).ToString("dd/MM/yyyy");
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            ...
            if (Agenda.ExisteVistoria(mtxtDataVis.Text, mtxtHora.Text.Substring(0, 2) + ":" + mtxtHora.Text.Substring(2, 2), lstCdPessoa[cmbPessoa.SelectedIndex]) == "")
            {
                ...
                if (Caminho == 1)
                {
                    ...
                    while (DR.Read())
                    {
                        ...
                        if (DR.GetDateTime(0) == vistoria.monthCalendar1.TodayDate && DR.GetTimeSpan(2) < TimeSpan.Parse(hora) || DR.GetDateTime(0) < vistoria.monthCalendar1.TodayDate)
                        {
                            if (DR[4].ToString() == "" && DR[5].ToString() == "")
                            {
                                if (DR[7].ToString() != "")
                                {
                                    if (DR[7].ToString() != "1")
                                    {
                                        vistoria.dgvListas.Rows.Add(DR.GetDateTime(0).ToString("dd/MM/yyyy"), DR[1].ToString(), "Dono de um abrigo inativo");
                                    }
                                    else
                                    {
                                        vistoria.dgvListas.Rows.Add(DR.GetDateTime(0).ToString("dd/MM/yyyy"), DR[1].ToString(), "Dono de um abrigo Ativo");
                                    }
                                }
                                else
                                {
                                    if (DR[8].ToString() != "")
                                    {
                                        if (DR[7].ToString() != "1")
                                        {
                                            vistoria.dgvListas.Rows.Add(DR.GetDateTime(0).ToString("dd/MM/yyyy"), DR[1].ToString(), "Adotante desbloqueado");
                                        }
                                        else
                                        {
                                            vistoria.dgvListas.Rows.Add(DR.GetDateTime(0).ToString("dd/MM/yyyy"), DR[1].ToString(), "Adotante bloqueado");
                                        }
                                    }
                                    else
                                    {
                                        vistoria.dgvListas.Rows.Add(DR.GetDateTime(0).ToString("dd/MM/yyyy"), DR[1].ToString(), "");
                                    }
                                }
            ...
            {
                MessageBox.Show(Agenda.ExisteVistoria(mtxtDataVis.Text, mtxtHora.Text.Substring(0, 2) + ":" + mtxtHora.Text.Substring(2, 2), lstCdPessoa[cmbPessoa.SelectedIndex]), this.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        #endregion
